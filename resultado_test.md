...
En un programa real de 5 ejes simultáneos, el controlador procesa miles de líneas G1 por segundo. Cada línea requiere calcular `sin(A)`, `cos(A)`, `sin(C)`, `cos(C)` — operaciones costosas en un microcontrolador sin FPU rápida.
...
El test de coherencia confirma que `transform_to_cartesian()` (Motor→TCP) es la inversa exacta de `transform_from_cartesian()` (TCP→Motor), con error < 0.002mm.
